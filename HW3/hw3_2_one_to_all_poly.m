...
[w1,b1,sv1] = svm(phi_train_x,[ones(40,1);-1*ones(80,1)],C,tol);
[w2,b2,sv2] = svm(phi_train_x,[-1*ones(40,1);ones(40,1);-1*ones(40,1)],C,tol);
[w3,b3,sv3] = svm(phi_train_x,[-1*ones(80,1);ones(40,1)],C,tol);
supv1 = sqrt([sv1(:,1),sv1(:,3)]);
supv2 = sqrt([sv2(:,1),sv2(:,3)]);
supv3 = sqrt([sv3(:,1),sv3(:,3)]);
%Get each training y
y1_train = phi_train_x*w1+b1;
y2_train = phi_train_x*w2+b2;
y3_train = phi_train_x*w3+b3;
%% Classify
...
y1_test = phi_test_x*w1+b1;
y2_test = phi_test_x*w2+b2;
y3_test = phi_test_x*w3+b3;
%% Classify
...
phi_poly_feature = [plot_feature(:,1).^2,sqrt(2)*plot_feature(:,1).*plot_feature(:,2),plot_feature(:,2).^2];
y1_feature = phi_poly_feature*w1+b1;
y2_feature = phi_poly_feature*w2+b2;
y3_feature = phi_poly_feature*w3+b3;
